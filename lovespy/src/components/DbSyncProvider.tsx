    ...
    fetch("/api/db/sync", { signal: controller.signal })
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          console.warn("Database sync response was not successful. Falling back to local storage:", data.error);
          setLoaded(true);
        }
      })
      .catch((err) => {
        clearTimeout(timeoutId);
        console.warn("Failed to sync DB from MongoDB. Falling back to local storage cache:", err);
        setLoaded(true);
      });
  }, []);
